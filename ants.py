...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while current_place is not beehive:
            ...
            within_range = self.min_range <= ref <= self.max_range
            has_bees = current_place.bees != empty
            
            if has_bees and within_range:
                target = rANTdom_else_none(current_place.bees)
                return target
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        fire_place = self.place
        
        # Reflected Damage to every bee in its place
        for bee in list(fire_place.bees):
            Insect.reduce_armor(bee, amount)
        
        # Reduce the FireAnt's armor by amount
        # This also remove the Fire Ant when it dies
        Ant.reduce_armor(self, amount)
        
        # Bees sustain more damage when the FireAnt dies
        if self.armor <= 0:
            for bee in list(fire_place.bees):
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        QueenAnt.queens += 1
        if QueenAnt.queens == 1:
            self.status = 'True Queen'
            self.buffed_Ants = []
        else:
            self.status = 'Impostor'
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        # Impostors die on their first actions
        if self.status == 'Impostor':
            Ant.reduce_armor(self, self.armor)
            self.remove_from(self.place)
                            
        # True Queen Attacks and Double the ants damage behind the true queen
        else:
            ThrowerAnt.action(self, gamestate)
            current = self.place
    
            while current is not None:
                #print(current.ant, 'is in', current.ant in self.buffed_Ants)
                if ( not(current.ant in self.buffed_Ants) and 
                     type(current.ant) != QueenAnt and
                     isinstance(current.ant, Ant) ):

                    current.ant.damage *= 2
                    self.buffed_Ants.append(current.ant)
                    
                    #print('buffed', self.buffed_Ants, 'damage', current.ant.damage)
    ...
    def reduce_armor(self, amount):
        ...
        # Reduce any Queen's armor
        Insect.reduce_armor(self, amount)
        
        # Bees win only if the True Queen died and Bee intruded the True Queen
        if self.status == 'True Queen':
            queen_died = self.armor <= 0
            bee_intruded = self.place.bees is not []
            
            if queen_died or bee_intruded:
                print('status', self.status)
                bees_win()
            
    def remove_from(self, place):
        if self.status == 'Impostor' and self.place is not None:
